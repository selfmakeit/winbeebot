...
//
// Media consist of InputMedia items (InputMediaPhotoTgbotapi, InputMediaVideoTgbotapi).
...
func prepareInputMediaParam(inputMedia interface{}, idx int) interface{} {
	...
	case InputMediaAudioTgbotapi:
		...
		return m
	case InputMediaDocumentTgbotapi:
		if m.Media.NeedsUpload() {
			m.Media = fileAttach(fmt.Sprintf("attach://file-%d", idx))
		}
